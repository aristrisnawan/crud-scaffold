class BarangsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @barang.save
        format.html { redirect_to @barang, notice: 'Barang was successfully created.', class: "text-success" }
  ...
  def update
    respond_to do |format|
      if @barang.update(barang_params)
        format.html { redirect_to @barang, notice: 'Barang was successfully updated.', class: 'text-success' }
  ...
  private
    ...
    def barang_params
      params.require(:barang).permit(:nama, :jenis, :harga)
    end
end
